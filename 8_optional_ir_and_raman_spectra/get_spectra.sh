...
# IR
awk '{print 1; print "#"; print "H", $0}' ../7_optional_dielectric_reponse_calculations/MACE_mu.txt > tmp.xyz
${IPI_GETACF} -ifile tmp.xyz -mlag 2048 -ftpad 2048 -ftwin cosine-hanning -oprefix mm -dt "2.0 femtosecond" -der
rm tmp.xyz

# Isotropic Raman
awk '{print 1; print "#"; print "H", $1 / 3, $5 / 3, $9 / 3}' ../7_optional_dielectric_reponse_calculations/MACE_alpha.txt > tmp.xyz
${IPI_GETACF} -ifile tmp.xyz -mlag 2048 -ftpad 2048 -ftwin cosine-hanning -oprefix L0L0 -dt "2.0 femtosecond"  -der
rm tmp.xyz

# Anisotropic Raman
awk '{print 2; print "#"; print "H", $2, $3, $4; print "H", $5, $6, $0}' ../7_optional_dielectric_reponse_calculations/MACE_alpha_sh.txt > tmp.xyz
